...
public class GA {
	...
	private double generationBestFitness=0;
	
	private Fitness fitnessFuncion = new noFitness();
	
	//constants
	public static final double weightRange = 30;
	...
	static final double Dt = 2;// max distance from the representator of a specie, to enter in that specie.
	static final double perturbanceRange = 0.05;//range of a perturbance mutation of a weight;
	//Probability
	static final double pDisabledP = 0.85;//probability that a gene is disabled, if one of the parent has it disabled
	static final double pDisable = 0.02;//probability that a gene is randomly disabled
	...
	static final double newConnection = 0.03;//probability of mutating a new, random, connection.
	...
	public void evolve(){
		...
		sortSpecies();
		double totalFitness =0;//sum of the shared fitness of every specie
		...
		for(int j=0;j<population.size();j++){
			for(int k=0;k<population.size();k++){
				if(j!=k && population.get(j)==population.get(k)){
		...
		for(Network n:population){
			for(int i=0;i<rep.size();i++){
				if(distance(n,rep.get(i))<=Dt){
					...
					n.setInSpecieID(species.get(i).getIndividualCounter());
					continue outerloop;
				}
			}
			rep.add(n);
...
//			}
		...
		if(r.nextDouble()<newConnection){
			...
			if(!existConnection(node1,node2,genome)){
